...
/* ----------------------------------------------------------------------
   Changed from fix_langevin.cpp May 14th 2016 version. Guang Shi, July 2016
------------------------------------------------------------------------- */
...
#include <string.h>
#include <stdlib.h>
#include "fix_langevin_overdamp.h"
...
FixLangevinOverdamp::FixLangevinOverdamp(LAMMPS *lmp, int narg, char **arg) :
  Fix(lmp, narg, arg)
{
...
FixLangevinOverdamp::~FixLangevinOverdamp()
{
  delete random;
  delete [] tstr;
  delete [] gfactor1;
...
int FixLangevinOverdamp::setmask()
{
  int mask = 0;
  mask |= POST_FORCE;
...
void FixLangevinOverdamp::setup(int vflag)
{
...
void FixLangevinOverdamp::post_force(int vflag)
{
...
void FixLangevinOverdamp::post_force_respa(int vflag, int ilevel, int iloop)
{
...
void FixLangevinOverdamp::post_force_templated()
#else
void FixLangevinOverdamp::post_force_untemplated
...
void FixLangevinOverdamp::compute_target()
{
  int *mask = atom->mask;
  int nlocal = atom->nlocal;
...
void FixLangevinOverdamp::omega_thermostat()
{
  double gamma1,gamma2;
  ...
  for (int i = 0; i < nlocal; i++) {
    if ((mask[i] & groupbit) && (radius[i] > 0.0)) {
      ...
      gamma2 = sqrt(inertiaone) * sqrt((20.0/3.0)*boltz/t_period/dt/mvv2e) / ftm2v;
      //gamma1 *= 1.0/ratio[type[i]];
      gamma2 *= 1.0/sqrt(ratio[type[i]]) * tsqrt;
      tran[0] = gamma2*(random->gaussian());
      tran[1] = gamma2*(random->gaussian());
      tran[2] = gamma2*(random->gaussian());
      torque[i][0] += tran[0];
      torque[i][1] += tran[1];
      torque[i][2] += tran[2];
    }
...
void FixLangevinOverdamp::angmom_thermostat()
{
  double gamma1,gamma2;
  ...
  for (int i = 0; i < nlocal; i++) {
    if (mask[i] & groupbit) {
      ...
      gamma2 = sqrt(ascale*2.0*boltz/t_period/dt/mvv2e) / ftm2v;
      //gamma1 *= 1.0/ratio[type[i]];
      gamma2 *= 1.0/sqrt(ratio[type[i]]) * tsqrt;
      ...
      tran[0] = sqrt(inertia[0])*gamma2*(random->gaussian());
      tran[1] = sqrt(inertia[1])*gamma2*(random->gaussian());
      tran[2] = sqrt(inertia[2])*gamma2*(random->gaussian());
      torque[i][0] += tran[0];
      torque[i][1] += tran[1];
      torque[i][2] += tran[2];
    }
...
void FixLangevinOverdamp::end_of_step()
{
  if (!tallyflag) return;
...
void FixLangevinOverdamp::reset_target(double t_new)
{
...
void FixLangevinOverdamp::reset_dt()
{
  if (atom->mass) {
    for (int i = 1; i <= atom->ntypes; i++) {
      gfactor2[i] = sqrt(atom->mass[i]) *
...
int FixLangevinOverdamp::modify_param(int narg, char **arg)
{
...
double FixLangevinOverdamp::compute_scalar()
{
...
void *FixLangevinOverdamp::extract(const char *str, int &dim)
{
  dim = 0;
...
double FixLangevinOverdamp::memory_usage()
{
  double bytes = 0.0;
...
void FixLangevinOverdamp::grow_arrays(int nmax)
{
...
void FixLangevinOverdamp::copy_arrays(int i, int j, int delflag)
{
...
int FixLangevinOverdamp::pack_exchange(int i, double *buf)
{
...
int FixLangevinOverdamp::unpack_exchange(int nlocal, double *buf)
{
