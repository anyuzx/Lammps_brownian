...
FixBDSRKOMP::FixBDSRKOMP(LAMMPS *lmp, int narg, char **arg) :
  FixBDSRK(lmp, narg, arg) { }

/* ----------------------------------------------------------------------
   allow for both per-type and per-atom mass
------------------------------------------------------------------------- */

void FixBDSRKOMP::initial_integrate(int vflag)
{
...
void FixBDSRKOMP::final_integrate()
{
  // update x of atoms in group
...
#pragma omp parallel for private(i) default(none) schedule(static)
    ...
    for (i = 0; i < nlocal; i++)
      if (mask[i] & groupbit) {
        const double dtfm = dtf / rmass[i];
        x[i].x += dtv * 0.5 * dtfm * (f[i].x - fprev[i][0]);
        x[i].y += dtv * 0.5 * dtfm * (f[i].y - fprev[i][1]);
        x[i].z += dtv * 0.5 * dtfm * (f[i].z - fprev[i][2]);
      }
