...
FixStyle(langevin/overdamp,FixLangevinOverdamp)

#else

#ifndef LMP_FIX_LANGEVIN_OVERDAMP_H
#define LMP_FIX_LANGEVIN_OVERDAMP_H

#include "fix.h"

namespace LAMMPS_NS {

class FixLangevinOverdamp : public Fix {
 public:
  FixLangevinOverdamp(class LAMMPS *, int, char **);
  virtual ~FixLangevinOverdamp();
  int setmask();
  void init();
  void setup(int);
